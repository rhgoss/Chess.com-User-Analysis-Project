...
'''
From this point forwards, the code written is my own. With that said, I did use online resources to find the appropriate syntax and logic for functions I was unfamiliar with. Additionally, some of the code ended up being very similar to the code from the example project that inspired me to make this, but that was bound to happen with the similarity between the two projects and the alike problems that were bound to arise. 
...
#I had no July values in my data set, after some thinking I believe that I likely played little to no games in the summer and also may have lost games from july from the pages chess.com wouldn't let me access
...
# I noticed that this scatter plot appears to follow a bell curve, which makes sense because as the number of games in a days increases we would expect the sample variance to decrease and see more values concentrated to the population mean (0.5). 
mean = 0.5
sd = 0.2
...
# The first regression model is with 3199 games not analyzed for accuracy, and the second consists of 146 games that I was able to analyze for accuracy.
...
# In my regression course, we used R, so I spent time learning to use sklearn in python to create my logistic regression model. 
...
# While I had not used anything similar to recursive feature estimation, it was interesting to try it out on my models to see what appear to better 'features' for my model. 
...
#interestingly, the three best ranked 'features' for this regression are me playing as black, my accuracy and my opponent's accuracy, but me playing black is not significant in the model while the 2 of the 3 features ranked lower are significant.
rfe2 = RFE(logistic2)
...
# Some interesting things to note was that there were multiple different variables that I thought might be significant in a model or meaningful in some way, but did not end up having much/any impact on the models. For example, I explored seeing if the time of year impacted my win probability (breaking up the year month by month and also into quarters) but that did not matter. I thought I would be more motivated in months when I had chess club during school, but I was wrong. Additionally, once my accuracy and my opponent's accuracy were taken into account in my second regression model, my starting color was no longer statistically significant. 
